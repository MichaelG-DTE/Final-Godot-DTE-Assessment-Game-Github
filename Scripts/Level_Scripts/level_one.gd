...
#Checks for the quit input, and keeps the timer going constantly
func _process(delta):
...
#instaniates the laser, from the signal that the player emits
...
#killing enemies provides points
func _on_enemy_killed(points):
	GlobalVar.score += points

#after you die, the game over screen appears and the game over music plays
...
#After the end of the wave, removes one from the wave counter, and then plays the same animation as on_ready
func _on_end_of_wave_timeout():
	...
	if waves == 2:
		...
		#once the level is complete, shows the level complete screen and plays the music
		if waves == 0:
			print("level finished")
			lcs.set_score(GlobalVar.score)
